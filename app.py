...
from datetime import datetime, date # Ensure 'date' is also imported for datetime.date checks
import gspread
...
def load_video_data():
    ...
    try:
        ...
        # Convert date columns to date objects, coercing errors to NaT, then getting just the date part
        # Handle potential empty strings or various date formats
...
def save_video_entry(amount, currency, client, paid, video_name, length_min, initial_date, deadline):
    ...
    try:
        ...
        for col in EXPECTED_HEADERS:
            ...
            # Robustly handle date/datetime conversion to string for Google Sheets
            if pd.isna(value): # Check for pandas NaT or None
                new_entry_values.append('') # Append empty string for missing dates/datetimes
            elif isinstance(value, datetime):
                new_entry_values.append(value.strftime("%Y-%m-%d %H:%M:%S"))
            elif isinstance(value, date): # Use 'date' type directly here
...
def update_entire_sheet(df_to_update):
    ...
    try:
        ...
        # Ensure pd.NaT values are converted to empty strings
...
def main():
    ...
    # --- Admin: Edit Entries Section (Now with separate tables for USD and PKR) ---
        ...
        # Separate DataFrames for USD and PKR
        usd_df_original = df[df['currency'] == 'USD'].copy()
        pkr_df_original = df[df['currency'] == 'PKR'].copy()

        # --- USD Table ---
        st.markdown("### 💵 USD Entries")
        if usd_df_original.empty:
            st.info("No USD entries available.")
            edited_usd_df_result = pd.DataFrame(columns=EXPECTED_HEADERS) # Empty DataFrame if no USD data
        else:
            edited_usd_df_result = st.data_editor(
                usd_df_original,
                ...
                column_config={
                    ...
                    "currency": st.column_config.SelectboxColumn("Currency", options=["USD", "PKR"], disabled=True), # Currency is fixed for this table
                    "client": st.column_config.TextColumn("Client"),
                    "paid": st.column_config.CheckboxColumn("Paid"),
                    "video_name": st.column_config.TextColumn("Video Name"),
                    "length_min": st.column_config.NumberColumn("Length (min)", format="%.1f", disabled=True), # Length disabled for USD
                ...
                num_rows="dynamic",
                hide_index=False # Keep index visible for debugging and accurate row identification
            )
        ...
        if pkr_df_original.empty:
            st.info("No PKR entries available.")
            edited_pkr_df_result = pd.DataFrame(columns=EXPECTED_HEADERS) # Empty DataFrame if no PKR data
        else:
            edited_pkr_df_result = st.data_editor(
                pkr_df_original,
        ...
        if st.button("Save All Changes to Sheet", key="save_admin_changes_btn"):
            all_edited_rows = {}
            all_added_rows = []
            all_deleted_rows_indices = set()

            # Process USD table changes
            if "admin_data_editor_usd" in st.session_state:
                usd_state = st.session_state["admin_data_editor_usd"]
                all_edited_rows.update(usd_state.get("edited_rows", {}))
                all_added_rows.extend(usd_state.get("added_rows", []))
                all_deleted_rows_indices.update(usd_state.get("deleted_rows", set()))

            # Process PKR table changes
            if "admin_data_editor_pkr" in st.session_state:
                pkr_state = st.session_state["admin_data_editor_pkr"]
                all_edited_rows.update(pkr_state.get("edited_rows", {}))
                all_added_rows.extend(pkr_state.get("added_rows", []))
                all_deleted_rows_indices.update(pkr_state.get("deleted_rows", set()))
            
            # Start with a copy of the original full DataFrame
            updated_df = df.copy() 

            # 1. Handle Deletions first
            if all_deleted_rows_indices:
                updated_df = updated_df.drop(index=list(all_deleted_rows_indices), errors='ignore')
                st.info(f"Deleted {len(all_deleted_rows_indices)} rows.")

            # 2. Handle Edits
            for idx, changes in all_edited_rows.items():
                # Ensure the index exists in the current DataFrame before attempting to update
                if idx in updated_df.index:
                    ...
                    st.warning(f"Skipping edit for row with original index {idx} as it might have been deleted.")
            ...
            if all_added_rows:
                ...
                # Ensure data types for added rows
                ...
                added_df['datetime'] = pd.to_datetime(added_df[col], errors='coerce') # Fix: use 'datetime' column for conversion

                # Fill any missing expected columns for added rows with sensible defaults
                for col in EXPECTED_HEADERS:
                ...
                # Ensure added_df has the same columns and order as EXPECTED_HEADERS
            ...
            if all_edited_rows or all_added_rows or all_deleted_rows_indices:
                try:
